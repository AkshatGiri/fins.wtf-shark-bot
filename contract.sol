...
interface IFinsBeachBar {
    function balanceOf(address _address, uint256 tokenID)
        external
        returns (uint256);
    ...
    Ownable
{
    ...
    function isFinsMember(address _vault) public returns (bool) {
        ...
        if (_vault != address(0)) {
            ...
            ).checkDelegateForContract(msg.sender, _vault, finsContractAddress);
            require(isDelegateValid, "invalid delegate-vault pairing");
            mintAddress = _vault;
        }

        if (
            IFinsBeachBar(finsContractAddress).balanceOf(
                mintAddress,
                FINS_BEACH_BAR_HOLOCAT_TOKEN_ID
            ) > 0
        ) {
            return true;
        }
        ...
        for (uint256 i; i <= MAX_FINS_BEACH_BAR_TOKEN; ++i) {
            if (
                IFinsBeachBar(finsContractAddress).balanceOf(mintAddress, i) > 0
            ) {
                return true;
            }
        }
    ...
    function _idToMint() internal returns (uint256) {
        ...
        if (
            msg.value >= generousTip && _kindaRandom(GENEROUS_TIPPER_ODDS) == 10
        ) {
